...
// This program makes a list of orbits of the action of the group on bases. It
// starts from the output of the code q-short_l.c.
...
{
  int i, j;
  char b[B];    // b[i] is the index of the i-th basis after permutation
...
int isgood(char *l, int z, int in1,
           int in2) // used in ispossible(), checks whether two bases in1 and
                    // in2 can be as they are given by l
{
  ...
  for (j = 0; j < R;
       j++) // different permutations of the first basis (chirotopes are
            // alternating and we do not store all the bases)
  {
    for (i = 0; i < R; i++) // axiom B2'
    {
      ...
      if (s1 && s2) // the x's and the y's are non-zero bases, so we want to
                    // find an i from Axiom B2'
      {

        t1 = orind[ind(x)];
        t2 = orind[ind(y)];

        if (t1 >= z && t2 >= z) // we don't know the signs of these bases - i
                                // could be among them
        {
          found++;
          break;
        ...
        if ((t1 >= z && s2) ||
            (t2 >= z &&
             s1)) // one basis is not zero and we do not know anything about the
                  // other one, that one could be i
        {
          found++;
          break;
        }
        if (t1 < z && t2 < z && l[t1] &&
            l[t2]) // both bases are not zero, we found i (possibly, we still
                   // don't know the sign)
        {
          found++;
          break;
...
int ispossible(char *l,
               int z) // checks for the list of signs of tests of length z
                      // whether it can be extended to a full list of signs in
                      // whole_l.c. Most sign combination do not lead to a
                      // chirotope and that is what we want to exclude here.
{
  int i, j, k, p, q, limit;

  for (i = 0; i < z;
       i++) // check only for the orbits that have not been checked yet
    if (l[i])
      for (j = 0; j < sizeofgroup; j++) {
        limit = sizeofgroup;
        for (p = 0; p <= i; p++) {
          ...
          for (q = 0; q < limit; q++)
            if (l[orind[orbits[p][q]]] != 0)
              if (isgood(l, z, orbits[i][j], orbits[p][q]) == 0)
                return 0;
        }
      }
...
void makepossible(
    char *l, int i,
    int z) // z is the length of l that we are making, in final version z=testnr
...
main() {
  ...
  while (k == 1) {
    for (i = 0; i < size; i++)
      printf("%3d", l[i]);
    putchar('\n');
    makepossible(l, size, new_size);
    for (i = 0; i < size; i++)
      k = fscanf(tfile, "%d", &l[i]);
  }

  end = clock();
  printf("N=%d, R=%d, sizeofgroup=%d, old size=%d, new size=%d, %.2f min, %llu "
         "l's\n",
         N, R, sizeofgroup, size, new_size,
         ((double)(end - begin)) / CLOCKS_PER_SEC / 60, made);
